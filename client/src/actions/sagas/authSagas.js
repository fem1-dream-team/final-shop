...
function * createNewUserWorker (action) {
	try {
		...
		yield call(() => axios.post('https://fem1-candy-factory.herokuapp.com/api/register', action.payload));
...
function * loginUserWorker (action) {
	try {
		yield put(isLoading(true))
		const response = yield call(() => axios.post('https://fem1-candy-factory.herokuapp.com/api/login', action.payload));
